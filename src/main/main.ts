...
export class ElectronApp {
  ...
  constructor() {
    ...
    this.pluginManager = new PluginManager(this.databaseService, this.securityManager)
  ...
  private setupIpcHandlers() {
    ...
    ipcMain.handle(IPC_CHANNELS.DB_GET_USER_PLUGINS, async (_, userId: string) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DB_UPDATE_SETTINGS, async (_, key: string, value: any) => {
      return await this.databaseService.updateSetting(key, value)
    })

    ipcMain.handle(IPC_CHANNELS.DB_UPDATE_USER, async (_, userId: string, userData: any) => {
      try {
    ...
    // Plugin management
    ipcMain.handle(IPC_CHANNELS.PLUGIN_INSTALL, async (_, pluginPath: string) => {
      try {
    ...
    ipcMain.handle(IPC_CHANNELS.PLUGIN_UNINSTALL, async (_, pluginId: string) => {
      try {
    ...
    // Security
    ipcMain.handle(IPC_CHANNELS.SECURITY_CHECK_PERMISSION, async (_, pluginId: string, permission: string) => {
      return await this.securityManager.checkPermission(pluginId, permission)
    })

    ipcMain.handle(IPC_CHANNELS.SECURITY_REQUEST_PERMISSION, async (_, pluginId: string, permission: string) => {
      return await this.securityManager.requestPermission(pluginId, permission)
    })

    // Plugin communication
    ipcMain.handle(IPC_CHANNELS.PLUGIN_MESSAGE, async (_, pluginId: string, message: any) => {
      return await this.pluginManager.sendMessageToPlugin(pluginId, message)
    })

    // User plugin management
    ipcMain.handle(IPC_CHANNELS.USER_PLUGIN_ENABLE, async (_, userId: string, pluginId: string) => {
      try {
    ...
    ipcMain.handle(IPC_CHANNELS.USER_PLUGIN_DISABLE, async (_, userId: string, pluginId: string) => {
      try {
