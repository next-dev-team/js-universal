...
import { Plugin, PluginManifest, IPC_CHANNELS } from '../../shared/types'
...
export class PluginManager {
  ...
  constructor(databaseService: DatabaseService, securityManager: SecurityManager) {
  ...
  async installPlugin(sourcePath: string): Promise<Plugin> {
    ...
    if (!sourcePath.includes('/') && !sourcePath.includes('\\')) {
      ...
      try {
        ...
        console.log(`Resolved plugin ID "${sourcePath}" to path: ${actualSourcePath}`)
      } catch (error) {
        console.error(`Plugin directory not found for ID "${sourcePath}": ${potentialPath}`)
        throw new Error(`Plugin "${sourcePath}" not found in plugins directory`)
      }
    }
  ...
  async launchPlugin(pluginId: string): Promise<void> {
    ...
    const pluginWindow = await this.pluginSandbox.createSandbox(pluginId, manifest)
  ...
  async closePlugin(pluginId: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      return {
        success: false,
        message: `Failed to close plugin: ${error instanceof Error ? error.message : 'Unknown error'}`
      }
    }
  }
