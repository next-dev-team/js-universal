...
export class PluginAPIBridge {
  ...
  private pluginContexts: Map<string, PluginContext & { storage: Map<string, any> }> = new Map()
  ...
  private setupAPIHandlers(): void {
    ...
    ipcMain.handle('plugin-api:storage-set', async (event, key: string, value: any) => {
      const pluginId = this.getPluginIdFromEvent(event)
    ...
    // Notification API
    ipcMain.handle('plugin-api:show-notification', async (event, title: string, body: string, options?: any) => {
      const pluginId = this.getPluginIdFromEvent(event)
    ...
    // Network API
    ipcMain.handle('plugin-api:fetch', async (event, url: string, options?: RequestInit) => {
      const pluginId = this.getPluginIdFromEvent(event)
    ...
    ipcMain.handle('plugin-api:write-file', async (event, filePath: string, content: string) => {
      const pluginId = this.getPluginIdFromEvent(event)
      if (!pluginId || !this.hasPermission(pluginId, 'filesystem')) {
        throw new Error('File system permission denied')
      }
    ...
    ipcMain.handle('plugin-api:send-message', async (event, targetPluginId: string, message: any) => {
    ...
    ipcMain.handle('plugin-api:broadcast-message', async (event, message: any) => {
      const senderPluginId = this.getPluginIdFromEvent(event)
      if (!senderPluginId) return false
    ...
    // Permission API
    ipcMain.handle('plugin-api:request-permission', async (event, permission: string) => {
      const pluginId = this.getPluginIdFromEvent(event)
      if (!pluginId) return false
    ...
    ipcMain.handle('plugin-api:has-permission', async (event, permission: string) => {
      const pluginId = this.getPluginIdFromEvent(event)
      if (!pluginId) return false
    ...
    // Utility API
    ipcMain.handle('plugin-api:log', async (event, level: string, message: string, ...args: any[]) => {
      const pluginId = this.getPluginIdFromEvent(event)
      const timestamp = new Date().toISOString()
      
      console.log(`[${timestamp}] [Plugin:${pluginId}] [${level.toUpperCase()}] ${message}`, ...args)
  ...
  private async persistPluginStorage(pluginId: string, key: string, value: any): Promise<void> {
    try {
