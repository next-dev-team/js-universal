...
export class SecurityManager {
  ...
  private initializeDefaultPermissions() {
    ...
    this.pluginPermissions.set('default', new Set([
      'storage',
      'notifications'
    ]))
  }
  ...
  private async showPermissionDialog(pluginId: string, permission: string): Promise<boolean> {
    const result = await dialog.showMessageBox({
      ...
      detail: `The plugin wants to access: ${this.getPermissionDescription(permission)}\n\nDo you want to allow this?`,
      cancelId: 1
    })
    ...
    try {
      ...
      if (manifest.permissions && Array.isArray(manifest.permissions)) {
        const validPermissions = [
          'storage', 'notifications', 'network', 'filesystem',
      ...
      if (manifest.main) {
        const entryPath = path.resolve(path.dirname(manifestPath), manifest.main)
        try {
  ...
  async sanitizePluginPath(pluginPath: string): Promise<{
    ...
    try {
      ...
      return {
        isValid: false,
        error: `Invalid plugin path: ${error instanceof Error ? error.message : 'Unknown error'}`
      }
    }
  }
  ...
  private createNotificationsAPI(pluginId: string) {
    return {
      show: async (title: string, body: string, options?: NotificationOptions) => {
        ...
        console.log(`[Plugin ${pluginId}] Notification:`, { title, body, options })
      }
    }
  }
