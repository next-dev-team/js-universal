  ...
  async loadPlugin(pluginId: string, pluginPath: string): Promise<void> {
    ...
    if (entryPoint.endsWith(".js")) {
      ...
      const tempHtmlPath = join(
        this.pluginContexts.get(pluginId)!.tempPath,
        "index.html"
      );
...
<body>
  ...
  private setupIPCHandlers(): void {
    // Plugin API handler
    ipcMain.handle(
      IPC_CHANNELS.PLUGIN_API_CALL,
      async (event, { pluginId, api, method, args }) => {
        try {
  ...
  private async handleStorageAPI(
  ...
  private async handleNetworkAPI(
    context: PluginContext,
    method: string,
    args: any[]
  ): Promise<any> {
  ...
  private async handleFilesystemAPI(
    context: PluginContext,
    method: string,
    args: any[]
  ): Promise<any> {
