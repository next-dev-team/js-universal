...
import { Plugin, UserPlugin, AppSettings, User } from '../../shared/types'
...
interface AppState {
  ...
  // Plugin actions
  installPlugin: (pluginPath: string) => Promise<{ success: boolean; message: string }>
  ...
  enableUserPlugin: (userId: string, pluginId: string) => Promise<{ success: boolean; data?: any; error?: string }>
  disableUserPlugin: (userId: string, pluginId: string) => Promise<{ success: boolean; data?: any; error?: string }>
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  uninstallPlugin: async (pluginId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const runningPlugins = get().runningPlugins.filter(id => id !== pluginId)
        set({ runningPlugins })
      }
      return result
    } finally {
  ...
  enableUserPlugin: async (userId: string, pluginId: string) => {
    try {
      const result = await window.electronAPI.enableUserPlugin(userId, pluginId)
      if (result) {
        await get().loadUserPlugins()
      }
  ...
  disableUserPlugin: async (userId: string, pluginId: string) => {
    try {
      const result = await window.electronAPI.disableUserPlugin(userId, pluginId)
      if (result) {
        await get().loadUserPlugins()
        // Remove from running plugins if it was running
        const runningPlugins = get().runningPlugins.filter(id => id !== pluginId)
  ...
  loadUserPlugins: async () => {
    try {
      ...
      if (currentUser) {
        const userPlugins = await window.electronAPI.getUserPlugins(currentUser.id)
        set({ userPlugins })
  ...
  loadSettings: async () => {
    try {
      const appSettings = await window.electronAPI.getSettings() as unknown as AppSettings | null
      if (appSettings) {
  ...
  updateUser: async (user: Partial<User>) => {
    try {
      const currentUser = get().currentUser
      if (currentUser) {
        const updatedUser = await window.electronAPI.updateUser(currentUser.id, user)
