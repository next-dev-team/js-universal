...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getUserPlugins: (userId: string) => ipcRenderer.invoke(IPC_CHANNELS.DB_GET_USER_PLUGINS, userId),
  getSettings: () => ipcRenderer.invoke(IPC_CHANNELS.DB_GET_SETTINGS),
  updateSettings: (key: string, value: any) => ipcRenderer.invoke(IPC_CHANNELS.DB_UPDATE_SETTINGS, key, value),
  updateUser: (userId: string, user: any) => ipcRenderer.invoke(IPC_CHANNELS.DB_UPDATE_USER, userId, user),
  clearAppData: () => ipcRenderer.invoke(IPC_CHANNELS.DB_CLEAR_DATA),

  // Plugin management
  installPlugin: (pluginPath: string) => ipcRenderer.invoke(IPC_CHANNELS.PLUGIN_INSTALL, pluginPath),
  ...
  launchPlugin: (pluginId: string) => ipcRenderer.invoke(IPC_CHANNELS.PLUGIN_LAUNCH, pluginId),
  closePlugin: (pluginId: string) => ipcRenderer.invoke(IPC_CHANNELS.PLUGIN_CLOSE, pluginId),

  // User plugin management
  enableUserPlugin: (userId: string, pluginId: string) => ipcRenderer.invoke(IPC_CHANNELS.USER_PLUGIN_ENABLE, userId, pluginId),
  disableUserPlugin: (userId: string, pluginId: string) => ipcRenderer.invoke(IPC_CHANNELS.USER_PLUGIN_DISABLE, userId, pluginId),

  // Security
  checkPermission: (pluginId: string, permission: string) => 
    ipcRenderer.invoke(IPC_CHANNELS.SECURITY_CHECK_PERMISSION, pluginId, permission),
  requestPermission: (pluginId: string, permission: string) => 
    ipcRenderer.invoke(IPC_CHANNELS.SECURITY_REQUEST_PERMISSION, pluginId, permission),
  ...
  // External links
  openExternal: (url: string) => ipcRenderer.invoke(IPC_CHANNELS.SHELL_OPEN_EXTERNAL),
...
declare global {
  interface Window {
    electronAPI: {
      ...
      // Plugin management
      installPlugin: (pluginPath: string) => Promise<{ success: boolean; message: string; pluginId?: string }>
      ...
      launchPlugin: (pluginId: string) => Promise<{ success: boolean; message: string }>
      closePlugin: (pluginId: string) => Promise<{ success: boolean; message: string }>
      ...
      // Plugin communication
      sendPluginMessage: (pluginId: string, message: any) => Promise<{ success: boolean; response?: any }>
