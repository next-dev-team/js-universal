...
export type EventListener<T = any> = (event: BaseEvent<T>) => void | Promise<void>;
...
export interface UIEvent extends BaseEvent {
  type: 'ui:modal:open' | 'ui:modal:close' | 'ui:notification:show' | 'ui:notification:hide';
}
...
export interface CacheEvent extends BaseEvent {
  type: 'cache:hit' | 'cache:miss' | 'cache:set' | 'cache:delete' | 'cache:clear';
  payload: {
    key: string;
    value?: any;
    ttl?: number;
...
export interface WebSocketEvent extends BaseEvent {
  type: 'ws:connect' | 'ws:disconnect' | 'ws:message' | 'ws:error' | 'ws:reconnect';
  payload: {
    url?: string;
    message?: any;
    error?: string;
...
export interface FileEvent extends BaseEvent {
  type: 'file:upload:start' | 'file:upload:progress' | 'file:upload:complete' | 'file:upload:error';
  payload: {
    fileId: string;
    fileName: string;
    fileSize?: number;
...
export interface ErrorEvent extends BaseEvent {
  type: 'error:js' | 'error:network' | 'error:validation' | 'error:auth' | 'error:unknown';
  payload: {
    message: string;
    stack?: string;
    code?: string | number;
...
export interface AnalyticsEvent extends BaseEvent {
  type: 'analytics:track' | 'analytics:page' | 'analytics:identify' | 'analytics:group';
  payload: {
    event?: string;
...
export interface MouseEvent extends BaseEvent<MouseEventData> {
  type: 'mouse:click' | 'mouse:dblclick' | 'mouse:mousedown' | 'mouse:mouseup' | 'mouse:mousemove';
}
