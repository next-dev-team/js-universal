...
import type { AppSettings, IDE, Project } from '../../main/services/database';
...
export interface ProjectManagerAPI {
  ...
  addIDE(
    ide: Omit<IDE, 'id'>,
  ): Promise<{ success: boolean; data?: IDE; error?: string }>;
  launchWithIDE(
    projectId: string,
    ideId: string,
  ): Promise<{ success: boolean; data?: boolean; error?: string }>;
  ...
  onProjectAdded(callback: (project: Project) => void): () => void;
}
...
const projectManagerAPI: ProjectManagerAPI = {
  ...
  scanDirectory: (path) =>
    ipcRenderer.invoke(IPC_CHANNELS.SCAN_DIRECTORY, path),
  importProjects: (projects) =>
    ipcRenderer.invoke(IPC_CHANNELS.IMPORT_PROJECTS, projects),
  launchProject: (projectId, ideId) =>
    ipcRenderer.invoke(IPC_CHANNELS.LAUNCH_PROJECT, projectId, ideId),
  openInExplorer: (projectId) =>
    ipcRenderer.invoke(IPC_CHANNELS.OPEN_IN_EXPLORER, projectId),
  openInTerminal: (projectId) =>
    ipcRenderer.invoke(IPC_CHANNELS.OPEN_IN_TERMINAL, projectId),
  ...
  launchWithIDE: (projectId, ideId) =>
    ipcRenderer.invoke(IPC_CHANNELS.LAUNCH_WITH_IDE, projectId, ideId),
  ...
  onProjectsUpdated: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.PROJECTS_UPDATED, handler);
  },

  onScanProgress: (callback) => {
    const handler = (_: Electron.IpcRendererEvent, progress: any) =>
      callback(progress);
  ...
  onScanComplete: (callback) => {
    const handler = (_: Electron.IpcRendererEvent, result: any) =>
      callback(result);
  ...
  onProjectAdded: (callback) => {
    const handler = (_: Electron.IpcRendererEvent, project: any) =>
      callback(project);
    ipcRenderer.on(IPC_CHANNELS.PROJECT_ADDED, handler);
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.PROJECT_ADDED, handler);
  },
