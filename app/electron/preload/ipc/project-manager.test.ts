...
describe('projectManagerAPI', () => {
  ...
  it('should call ipcRenderer.invoke with GET_PROJECTS when getProjects is called', async () => {
    ...
    (ipcRenderer.invoke as Mock).mockResolvedValueOnce({
      success: true,
      data: mockProjects,
    });
  ...
  it('should call ipcRenderer.invoke with ADD_PROJECT when addProject is called', async () => {
    ...
    (ipcRenderer.invoke as Mock).mockResolvedValueOnce({
      success: true,
      data: mockAddedProject,
    });

    const result = await projectManagerAPI.addProject({
      name: 'New Project',
      path: '/new/project',
      type: 'unknown',
      lastOpened: 0,
  ...
  it('should call ipcRenderer.invoke with UPDATE_PROJECT when updateProject is called', async () => {
    ...
    (ipcRenderer.invoke as Mock).mockResolvedValueOnce({ success: true });
  ...
  it('should call ipcRenderer.invoke with SCAN_DIRECTORY when scanDirectory is called', async () => {
    ...
    const mockScanResult = [
      { name: 'Scanned Project', path: '/scanned/project' },
    ];
    (ipcRenderer.invoke as Mock).mockResolvedValueOnce({
      success: true,
      data: mockScanResult,
  ...
  it('should call ipcRenderer.invoke with IMPORT_PROJECTS when importProjects is called', async () => {
    const projectsToImport = [
      { name: 'Imported Project', path: '/imported/project' },
    ];
    const mockImportedProjects = [
      { id: '3', createdAt: 'now', updatedAt: 'now', ...projectsToImport[0] },
    ];
    (ipcRenderer.invoke as Mock).mockResolvedValueOnce({
      success: true,
      data: mockImportedProjects,
    });

    const result = await projectManagerAPI.importProjects([
      {
  ...
  it('should call ipcRenderer.invoke with ADD_IDE when addIDE is called', async () => {
    ...
    const result = await projectManagerAPI.addIDE({
      name: 'Sublime Text',
      executablePath: '/path/to/sublime',
      arguments: [],
      isDefault: false,
      supportedTypes: [],
    });
  ...
  it('should call ipcRenderer.invoke with UPDATE_SETTINGS when updateSettings is called', async () => {
    ...
    const result = await projectManagerAPI.updateSettings({
