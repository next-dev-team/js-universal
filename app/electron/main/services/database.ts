...
class DatabaseService {
  ...
  getIDEs(): IDE[] {
    const rows = this.db.prepare('SELECT * FROM ides ORDER BY isDefault DESC, name').all() as any[];
    return rows.map(row => ({
      ...row,
  ...
  getSettings(): AppSettings {
    ...
    return {
      ...
      windowPosition: row.windowPosition ? JSON.parse(row.windowPosition) : null,
  ...
  updateSettings(updates: Partial<AppSettings>): boolean {
    ...
    const setClause = Object.keys(updateData)
      .map(key => {
        if (key === 'autoScanDirectories' || key === 'windowPosition') return `${key} = ?`;
        if (key === 'showOnStartup' || key === 'minimizeToTray') return `${key} = ?`;
        return `${key} = ?`;
      })
      .join(', ');

    const values = Object.entries(updateData).map(([key, value]) => {
      if (key === 'autoScanDirectories' || key === 'windowPosition') return JSON.stringify(value);
      if (key === 'showOnStartup' || key === 'minimizeToTray') return value ? 1 : 0;
      return value;
    });
