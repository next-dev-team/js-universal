...
export class WorkspaceScanner {
  ...
  async scanAndRegisterProjects(): Promise<void> {
    ...
    if (!fs.existsSync(this.appsDirectory)) {
      console.error('[WorkspaceScanner] Apps directory does not exist:', this.appsDirectory);
      return;
    }

    try {
      const entries = fs.readdirSync(this.appsDirectory, { withFileTypes: true });
      const projectDirs = entries.filter(entry => entry.isDirectory());

      console.log('[WorkspaceScanner] Found directories:', projectDirs.map(d => d.name));
      ...
      console.log('[WorkspaceScanner] Workspace scan completed. Registered projects:', Array.from(this.registeredProjects.keys()));
    } catch (error) {
   ...
   */
  private async scanAndRegisterProject(projectPath: string, projectName: string): Promise<void> {
    const packageJsonPath = path.join(projectPath, 'package.json');
    
    if (!fs.existsSync(packageJsonPath)) {
      console.log(`[WorkspaceScanner] Skipping ${projectName} - no package.json found`);
      return;
    }
    ...
    try {
      ...
      const hasDevServer = !!(packageJson.scripts?.dev || packageJson.scripts?.start);
      
      if (!hasDevServer) {
        console.log(`[WorkspaceScanner] Skipping ${projectName} - no dev server script found`);
        return;
      }
      ...
      console.log(`[WorkspaceScanner] Registering project ${projectName} with port ${devServerPort}`);
      ...
      console.error(`[WorkspaceScanner] Error processing project ${projectName}:`, error);
    }
  }
  ...
  private async registerProject(config: ProjectConfig): Promise<void> {
    ...
    const manifest = {
      ...
      description: config.packageJson.description || `${config.name} - Auto-detected workspace project`,
    ...
    try {
      const devResult = await this.pluginDevLoader.registerDevPlugin(devPluginConfig);
      console.log(`[WorkspaceScanner] Dev plugin registration for ${config.id}:`, devResult);
      ...
      if (config.hasDevServer && config.devServerPort) {
        ...
        const webviewResult = await this.pluginWebviewManager.registerWebviewPlugin(webviewConfig);
        console.log(`[WorkspaceScanner] Webview plugin registration for ${config.id}:`, webviewResult);

        // Auto-launch if successful
        if (webviewResult.success) {
          const launchResult = await this.pluginWebviewManager.launchWebviewPlugin(config.id);
          console.log(`[WorkspaceScanner] Auto-launch result for ${config.id}:`, launchResult);
        }
      }

      this.registeredProjects.set(config.id, config);
    } catch (error) {
      console.error(`[WorkspaceScanner] Failed to register project ${config.id}:`, error);
    }
  }
  ...
  private setupProjectFileWatcher(config: ProjectConfig): void {
    if (this.fileWatchers.has(config.id)) {
      console.log(`[WorkspaceScanner] File watcher already exists for ${config.id}`);
      return;
    }

    try {
      console.log(`[WorkspaceScanner] Setting up file watcher for ${config.id} at ${config.devPath}`);
      
      const watcher = fs.watch(config.devPath, { recursive: true }, (eventType, filename) => {
        if (!filename) return;

        // Filter for relevant file types
        const relevantExtensions = ['.html', '.js', '.ts', '.jsx', '.tsx', '.css', '.scss', '.json'];
        const ext = path.extname(filename);
        
        if (!relevantExtensions.includes(ext)) return;

        console.log(`[WorkspaceScanner] File change detected in ${config.id}: ${filename} (${eventType})`);
      ...
      console.log(`[WorkspaceScanner] File watcher setup completed for ${config.id}`);
    } catch (error) {
      console.error(`[WorkspaceScanner] Failed to setup file watcher for ${config.id}:`, error);
    }
  }
   ...
   */
  private async handleProjectFileChange(config: ProjectConfig, filename: string, eventType: string): void {
    console.log(`[WorkspaceScanner] Handling file change for ${config.id}: ${filename}`);
    ...
    try {
      ...
      if (this.pluginWebviewManager.isWebviewPluginRegistered(config.id)) {
        try {
          const reloadResult = await this.pluginWebviewManager.reloadWebviewPlugin(config.id);
          console.log(`[WorkspaceScanner] Webview reload result for ${config.id}:`, reloadResult);
        } catch (error) {
          console.error(`[WorkspaceScanner] Failed to reload webview for ${config.id}:`, error);
        }
      }
    } catch (error) {
      console.error(`[WorkspaceScanner] Failed to handle file change for ${config.id}:`, error);
    }
  }
  ...
  private setupDirectoryWatcher(): void {
    try {
      console.log('[WorkspaceScanner] Setting up directory watcher for new projects');
      
      const watcher = fs.watch(this.appsDirectory, { recursive: false }, async (eventType, filename) => {
        ...
        if (fs.existsSync(fullPath) && fs.statSync(fullPath).isDirectory()) {
          console.log(`[WorkspaceScanner] New project directory detected: ${filename}`);
      ...
      console.error('[WorkspaceScanner] Failed to setup directory watcher:', error);
    }
  }
  ...
  private extractPortFromDevScript(devScript?: string): number | undefined {
    ...
    for (const pattern of portPatterns) {
      ...
      if (match && match[1]) {
        ...
        console.log(`[WorkspaceScanner] Found port ${port} using pattern: ${pattern}`);
        return port;
      }
    }
    ...
    if (devScript.includes('webpack-dev-server')) {
      console.log(`[WorkspaceScanner] Using default webpack-dev-server port: 8080`);
      return 8080;
    }
    ...
    if (devScript.includes('react-scripts')) {
      console.log(`[WorkspaceScanner] Using default Create React App port: 3000`);
      return 3000;
    }
  ...
  cleanup(): void {
    ...
    for (const [projectId, watcher] of this.fileWatchers) {
      try {
        ...
        console.error(`[WorkspaceScanner] Error closing watcher for ${projectId}:`, error);
      }
    }
